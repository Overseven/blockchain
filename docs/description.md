...
Эмиссия монет ограничена объёмом 500 млн и управляется [майнингом](#майнинг) новых блоков, [аирдропами](#аирдроп) и наградами за участие в [голосовании](#голосование).

1/50 от общего объема эмиссии (10 млн.) остаётся за модератором, для поощерения активных участников сети и разработчиков [аирдропами](#аирдроп).

1/150 от всего объёма эмиссии (около 3 млн.) делится между основными разработчиками сети.

Остальные средства (около 487 млн.) находятся в свободном обращении, создаются при майнинге и голосовании.
...
#### Эмиссия
Помимо комиссий от транзакций, при создании блока генерируются новые монеты. За каждый блок генерируется одинаковая сумма TRY, равная `коэффициенту эмиссии`.

Коэффициент уменьшается в 2 раза каждые 1000 блоков.
...
- Опциональные:
  - `saveLocalChain <bool>` - битовый флаг (0/1, false/true), отвечающий за сохранение локальной копии блокчейна. При значении `false` валидатор не проверяет получаемые от ноды транзакции на корректность. Значение по умолчанию - `true`.
