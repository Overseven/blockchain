...
- [Голосование](#голосование)
  - [Начало голосования](#начало-голосования)
...
- минимальная сумма для заморозки для инициации голосования
- минимальный и максимальный порог для голосования
- способ учета голосов
- объём эмиссии монет при подписи блока
- процент согласий среди хокаге, для консенсуса при создании [чекпоинта](#чекпоинт)
- кол-во тиков при создании [чекпоинта](#чекпоинт)
- штраф для хокаге, который при консенсусе предоставил неверные данные
- размер коэффициента роста штрафной комиссии для хокаге
...
Для того, чтобы голосование началось, необходимо заморозить (сделать транзакцию на идентификатор) минимально необходимое кол-во токенов.
Замораживать токены для начала голосования может любой участник сети.

Если число замороженных токенов превышает пороговое значение, то с началом новой [эпохи](#эпоха) начинается голосование за изменения. Голосование длится 1 эпоху.
...
- ``coordinator <ip:port>`` - адрес [координатора](#координатор).
- ``cfgFile <path>`` - путь до файла конфигурации, в котором заданы параметры выше.
- ``nodeFee <float>`` - соотношение распределения комиссии между нодой и валидатором. Пример: если параметру задано значение `0.8`, то 80% от суммы комиссий, указанных в транзакциях достаются ноде. Не влияет на распределение новых монет. Допустимые значения: от `0.0` до `1.0`
