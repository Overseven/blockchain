...
- Виды транзакций:
  ...
  - аирдроп - генерация монет от лица модератора на указанный кошелек
...
### Валидатор
Валидатор (майнер) - участник сети, который подписывает блоки с транзакциями. Валидатор подключается к нодам, получает от них транзакции, требующие обработки.

Валидатор может хранить полную версию блокчейна для валидации транзакций.
...
Любой участник сети, имеющий кошелек, может инициировать перемещение своих средств на чужой кошелек.
...
Помимо комиссий от транзакций, при создании блока генерируются новые монеты. На начальной стадии разработки за каждый блок генерируется одинаковая сумма TRY, равная `кол-во транзакций в блоке * 10`.
