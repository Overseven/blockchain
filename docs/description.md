...
Нода должна заявить о себе через транзакцию специального типа, в которой должно быть указано распределение комиссии с подписываемых транхакций между самой нодой и валидатором.
...
- Свой адрес
- Комиссия, которую нода заплатит за подпись этой транзакции
- Коэффициент, отвечающий за то, какую часть от суммы всех комиссий заберет себе нода. Диапазон значений от `0.0` до `1.0`. Остальное получает валидатор.
- Временная метка
- Адрес ноды (которая будет подписывать эту транзакцию)
- Подпись
